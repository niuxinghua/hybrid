...
@interface HaierQRScanView ()<AVCaptureMetadataOutputObjectsDelegate>
...
- (void)closeSelf
{
    [self.delegate scanViewDidTouchCloseButton];
}
- (void)startScanning{
    if (![self statusCheck]) {
        return;
...
{
    if (!_closeButton) {
        _closeButton = [[UIButton alloc]initWithFrame:CGRectMake(10, 30, 44, 44)];
